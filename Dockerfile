...
RUN mkdir -p /build/bin \
    && CGO_ENABLED=1 GOOS=linux go build -a -v -i -o /build/bin/pillbox . \
    && sha256sum -b /build/bin/pillbox > /build/bin/SHA256SUMS
...
COPY --from=builder /build/bin/pillbox /home/vault/plugins/pillbox
