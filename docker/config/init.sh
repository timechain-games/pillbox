...
function configure() {
    banner "Installing pillbox plugin at $VAULT_ADDR..."
	SHA256SUMS=`cat /home/vault/plugins/SHA256SUMS | awk '{print $1}'`
	vault write sys/plugins/catalog/secret/pillbox \
		  sha_256="$SHA256SUMS" \
		  command="pillbox --ca-cert=$CA_CERT --client-cert=$TLS_CERT --client-key=$TLS_KEY"

	if [[ $? -eq 2 ]] ; then
	  echo "pillbox couldn't be written to the catalog!"
	  exit 2
	fi

	vault secrets enable -path=pillbox -plugin-name=pillbox plugin
	if [[ $? -eq 2 ]] ; then
	  echo "pillbox couldn't be enabled!"
	  exit 2
	fi
